...
| Consent + analytics | JS | `assets/analytics.js` + public pages | Marketing pages use Google advanced consent mode: before or after refusal, Google receives one sanitized cookieless `page_view`, while behavior events and Meta stay opt-in only. `/apply/` uses basic mode and blocks Google and Meta until opt-in. Events include `apply/contact/instagram/meetup_click`, `event_card_click`, `language_switch`, `section_view`, plus the `/apply/` funnel (`application_start`, `application_error`, `generate_lead`); Meta custom `ApplicationFormOpen`, standard `Contact` and `Lead`. Application saving is service delivery and remains independent of analytics consent |
