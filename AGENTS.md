...
| Pick which days the landing cards push | `assets/event-slots.js` `featured` | Card chips show featured days first via `highlightDates()` (same-day excluded); a featured day that is not a match day fails `tests/event-slots.test.js` |
...
| `#events` | section | `index.html` | Two-tier event cards from `CONTENT_MAP.events.cards` on the `panel` band shared with `#reviews`: sports nights (1-col mobile / 3-col desktop) carry featured-first date chips from `highlightDates()`, food & chill stay compact 3-col with an any-day line; open events link to `/events/*`, coming-soon cards show a toast |
