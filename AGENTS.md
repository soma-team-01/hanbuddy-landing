...
| Consent + analytics | JS | `index.html` script | GA4 (`G-MW7MFVL50G`) + Meta Pixel load only after opt-in; events: `apply/contact/instagram/meetup_click`, `event_card_click`, `language_switch`, `section_view`, plus the `/apply/` funnel (`application_start`, `application_error`, `generate_lead`); Meta custom `ApplicationFormOpen`, standard `Contact` and `Lead`. Refusing cookies blocks analytics only — the application itself still saves, because submitting a form is service delivery, not measurement |
