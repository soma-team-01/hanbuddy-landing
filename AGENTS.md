...
|   |-- korea-football/index.html # /events/korea-football — national team A-match (two venues), same template, no review section yet; the K League page was retired on 2026-09-04 and returns from git when FC Seoul home games resume in October
...
|   |-- application-measurement.js # canonical application funnel + submit de-duplication (browser + tests)
...
| Edit the application form | `apply/index.html` + `assets/apply-validation.js` | Validation is shared with the function — change it once, in the module. A field lives in four places: the markup, the EN/KO `fields` copy, the submit payload, and the shared validator. Adding one means updating the privacy notice and the sheet header in the same change; dropping one means writing `''` into its `buildRow` slot, never reflowing the fixed A:Q order (rows already in the sheet would shift a column) |
...
| Pick which days the landing cards push | `assets/event-slots.js` `featured` | Card chips start at the earliest live featured day and run forward in time only via `highlightDates()` (same-day excluded; earlier open days are deliberately hidden); a featured day that is not a match day fails `tests/event-slots.test.js` |
...
| Run a landing message A/B | `assets/landing-variant.js` + `CONTENT_MAP.hero.variants` (EN+KO) | Add the arm to `VARIANTS`, then its copy under `hero.variants` in both languages. No page split, no new route. Every page that can lead to the form loads the module and calls `propagateLinks`, or the value dies on the way to `/apply/` |
...
| `#events` | section | `index.html` | Two-tier event cards from `CONTENT_MAP.events.cards` on the `panel` band shared with `#reviews`: sports nights (1-col mobile / 3-col desktop) carry date chips that start at the featured day and run forward via `highlightDates()`, food & chill stay compact 3-col with an any-day line; open events link to `/events/*`, coming-soon cards show a toast |
| `#reviews` | section | `index.html` | Guest reviews on a flat `panel` band (no photos): aggregate rating chip + a manual arrow carousel of the approved quote cards from `assets/reviews-data.js`, oldest first, opening on card 2; rules in DESIGN.md "Guest Reviews" |
...
| `EVENT_SLOTS` | module | `assets/event-slots.js` | Single source for event dates, prices, and EN/KO slot labels. Expiry is judged in KST so the browser and the function agree regardless of the reader's timezone. `featured` days plus `highlightDates()` feed the landing sport cards' date chips (anchored at the earliest live featured day, forward in time only, same-day excluded, recurring events get none) |
| `GUEST_REVIEWS` | module | `assets/reviews-data.js` | Single source for approved quotes (EN/KO), their individual survey scores, and per-activity rating summaries. The landing carousel renders from it at runtime; the detail pages' static review sections must mirror it, which `tests/event-detail-reviews.test.js` enforces |
| `HanBuddyVariant` | module | `assets/landing-variant.js` | Google Search ad message A/B (2026-08-24). `resolve()` judges `?v=` against a whitelist and returns `null` for anything else, including a repeated parameter, so an unknown value renders the default screen instead of an unknown arm. `propagateLinks()` re-tags every site-internal anchor after a render, which is what keeps the arm attached through cards and detail pages to `/apply/`. External channels and `#anchors` are never touched |
...
| Consent + analytics | JS | `assets/analytics.js` + public pages | Marketing pages use Google advanced consent mode: before or after refusal, Google receives one sanitized cookieless `page_view`, while behavior events and Meta stay opt-in only. `/apply/` uses basic mode and blocks Google and Meta until opt-in. CTA keys map to GA4 events as `apply` → `application_form_open`, `contact`/`instagram` → `contact_click`, and `meetup` → `community_click`; other GA4 events include `select_content`, `language_switch`, `section_view`, plus the canonical `/apply/` funnel (`application_start`, `generate_lead`). `application_error` is auxiliary QA data, not a funnel stage. Application events carry only allowlisted page context; selected dates, guest count, source, and all other form values never enter analytics. A validated inbound event prefill may supply GA's predefined `content_id` with `content_type: experience`. Meta uses the custom event `ApplicationFormOpen` and the standard events `Contact` and `Lead`. Application saving is service delivery and remains independent of analytics consent |
...
- Completed runs usable as public proof: 2026-06-25 Jamsil KBO (Samsung Lions vs LG Twins), 2026-07-26 Jamsil KBO, and 2026-08-15 K League Football Night at Seoul World Cup Stadium (FC Seoul won 4:2; confirmed run by 유현님 on 2026-08-20). The long-standing Aug 1 Han River contradiction was resolved on 2026-08-20: the cancelled picnic came off the `/about` timeline. No Han River picnic has a confirmed operation record (the Aug 8 & 9 and Aug 15 & 16 dates passed without one), and the food nights and jjimjilbang have never been operated — their photos may be used only for upcoming/coming-up items or with neutral place-describing captions, never as completed-operation proof.
...
- The two baseball activities share the `KBO Baseball Night` stem; only the indoor/open-air qualifier differs. The open-air name dropped `at Jamsil` on 2026-08-22 (the venue made the landing card title wrap to two lines; the venue lives in the detail-page copy and the KO name still says 잠실). The Meetup listing keeps the old longer title until 유현님 updates it by hand. `KBO baseball` reads naturally in English the way `MLB baseball` does, and it captures both search terms: guests who already follow Korean baseball search `KBO`, everyone else searches `baseball`.
- `K League Football Night` carries no venue because there is only one football ground; add one only if a second appears. `Korea National Team Football Night` also carries no venue, for the opposite reason: the venue changes per match, so it lives in the slot label, not the name. Its detail `h1` takes the campaign subtitle `: Korea vs Uruguay & Ecuador`. It was `K League Football Day` while the card sat in "coming soon", renamed on 2026-08-06 when the 6:30 PM meeting time made "Day" wrong.
...
6. 2026-08-16 survey promo field (2026-08-15 K League run participant), single-sentence excerpt of a longer answer: "The guide was nice and it was like being with friends." / KO "가이드는 친절했고 친구들과 함께 있는 것 같았어요."
7. 2026-08-16 survey promo field (the other 2026-08-15 K League run participant): "Super fun experience and our guide was super kind, helpful and made the experience amazing!!" / KO "정말 재미있는 경험이었고, 가이드가 무척 친절하고 큰 도움이 되어 최고의 경험이 됐어요!!"
- Answers from the "What could be improved" survey field remain off-limits for public copy.
- The aggregate "4.8 / 5 average guest rating" is the mean of all post-event survey satisfaction scores to date (last recomputed 2026-08-22 over the six responses then in the sheet); update only by recomputing from the survey sheet.
- Individual scores are public since 2026-08-22, reversing the earlier never-expose rule (유현님 decision: enough reviews accumulated to show every score, the 4 included). Each approved quote carries its survey satisfaction score in `assets/reviews-data.js`, and the event detail pages print a star row per review plus a per-activity average and count (one-decimal mean over that activity's rated reviews via `ratingSummary()`; it is a different number from the site-wide 4.8 and the two need not match). Gocheok has no reviews of its own yet, so it borrows the Jamsil list under a visible source note. A quote with no survey response (the KakaoTalk message) has `rating: null` and never appears on a detail page: no review ships without its star row. `tests/event-detail-reviews.test.js` keeps the static detail markup equal to the data file.
...
- `assets/photos/kleague/*`, `assets/photos/samgyeopsal/*`, and `assets/photos/chimaek/*` are team-owned. All three kleague photos are from the 2026-08-15 run; the three pre-run stadium photos (`kleague-worldcup-day`, `kleague-night`, `kleague-kickoff`, personal shots from before HanBuddy) were deleted on 2026-08-22 when the run's own photos replaced them. `kleague-0815-scarf.webp` is a team phone shot (the four identifiable bystander faces in the crowd are Gaussian-blurred, no participant face appears). `kleague-0815-crew.webp` shows the two guests and their buddy in a selfie — both guests answered "Yes, I consent." to the photo-use question in the 2026-08-16 survey responses — with identifiable bystander faces in the crowd band Gaussian-blurred. `kleague-0815-dusk.webp` is a stands-level stadium view whose nearest identifiable bystander faces are Gaussian-blurred (rest are backs of heads or too distant); it also serves as the `#apply` backdrop slide. Three more 2026-08-15 photos were added on 2026-08-22 for the detail-page gallery: `kleague-0815-crew-night.webp` (the same consented guests and their buddy under the floodlights; the source selfie was saved mirrored by a Samsung front camera and was un-mirrored before deriving, with two bystander face areas Gaussian-blurred), `kleague-0815-tickets.webp` (match ticket stubs, cleared for as-is publication on 2026-08-22: the booking numbers belong to a finished match, the card number is masked on the ticket itself, and the payer name printed on the stub belongs to the team member who booked the tickets and cleared this use), and `kleague-0815-fourcut.webp` (Photoism four-cut booth strip of the run's guests and buddies in an FC Seoul frame, shared to the team Drive for this use). The crew selfie also fronts the hero polaroid scatter (`hp2`) since 2026-08-22, moving the first-baseball-night snap to `hp4` and retiring the Han River chimaek photo from the hero. Only `chimaek-table.webp` shows a person (a guest from the shoulders down, consent recorded 2026-08-10). Every other food photo was chosen or cropped so that no person appears at all: `samgyeopsal-table.webp` is cropped 24% off the top for exactly this reason, and shots where people were central to the frame were left out of the set. The CC0 jjimjilbang photos were deleted on 2026-08-10 with the activity.
...
- The site collects personal information through `/apply/` and stores it in the team's Google Sheet. What it collects, why, how long it is kept (6 months after the event), and the deletion contact (`zeroone.soma@gmail.com`) are all stated in the form, behind a required consent checkbox. Collecting a new field means updating that notice and the sheet header in the same change; a field we stop collecting leaves its sheet column in place and blank. Korean level and preferred payment method came off the form on 2026-08-20 (payment is settled in the confirmation contact instead), so columns I and L are now always empty. Nothing personal is ever committed to this repo.
- CTA URLs appear both as hardcoded anchors and in `CONFIG`; keep both aligned so the page works before JS enhancement.
- **Landing message A/B (`?v=local` / `?v=friends`, added 2026-08-24 for the Google Search ad test).** The hero headline and subtitle are the only things that change; the page, the form, and the analytics stack stay single. A request with no parameter, an unknown value, or a repeated `v` renders the default hero, so the rollback plan is "stop sending the parameter". `landing_variant` rides on `application_start` and `generate_lead` (`'none'` when absent) and deliberately stays out of the application payload: the sheet's column order is fixed and the arm is measurement, not service data. Arm copy lives in `CONTENT_MAP.hero.variants` in **both** languages; filling only one makes the experiment vanish for readers of the other. The `friends` arm must keep both halves of its pitch (the Korean buddy and the other guests on the same night) or it stops being distinguishable from the default message.
- **The hero subtitle is not folded away on phones** (reversed 2026-08-24). The 2026-08-18 mobile-density pass hid section lead paragraphs below `sm`, and the hero subtitle went with them. It now carries the ad message that the A/B test is judging, and ad traffic is mostly phones, so it stays visible and is written short enough to read there. Section leads in `#events` and `#reviews` keep the old rule.
...
- Do not reintroduce retired prices: the KBO nights are ₩65,000 since 2026-09-04 (₩60,000 from 2026-08-04 to 2026-09-03 and the ₩50,000 pilot framing are history); the national team night is ₩80,000.
- Do not reintroduce retired recruitment facts as current truth: "3 spots left", "8 seats booked", "first pilot recruitment", and the July 18/19 & 25/26 "Run 2" window are history. (Note: the KBO price was ₩60,000 from 2026-08-04 to 2026-09-03 and is ₩65,000 since 2026-09-04; ₩50,000 and the older "50,000 KRW" pilot framing are both retired.)
...
- Team repo rules: always branch + PR (main direct push is hook-blocked), squash merge, no AI co-author trailers in commits or PR bodies. PR titles and commit subjects must preserve the Conventional Commit prefix appropriate to the actual change (type and optional scope); do not force `feat`. Use `<type>(<optional-scope>): <Korean summary>` or, without scope, `<type>: <Korean summary>`. Only the description after the colon (`<Korean summary>`) must be a short, clear Korean summary of the actual change. Examples: `feat: 예약 기능 추가`, `fix(seo): 사이트 이름 신호 개선`, `docs: 제목 규칙 수정`.
