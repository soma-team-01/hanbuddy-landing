...
| `#reviews` | section | `index.html` | Guest reviews on a crossfading photo backdrop: aggregate rating chip and approved survey quotes (`CONTENT_MAP.reviews.cards`); backdrop rules in DESIGN.md "Reviews Backdrop" |
...
- A second approved public quote exists from the 2026-07-26 KBO run's post-event survey (the survey field explicitly asks for a review "we can use for promotion"): "Great experience to enjoy a baseball game with a local" / KO "로컬 버디와 함께 야구 경기를 즐길 수 있어 정말 좋은 경험이었어요".
- Quotes from the survey's "What did you like about today's experience?" field may also be published — 유현님 approved this on 2026-07-29 (the operator owns the guest relationship and makes this call). Currently published: "It was fun to watch the game and cheer together!" / KO "함께 응원하며 경기를 보는 게 정말 재미있었어요!" (2026-07-26 run, 5/5 rating). Only the sentence-initial capital is normalized; wording is verbatim. Answers from the "What could be improved" field remain off-limits for public copy.
...
- `assets/photos/kleague/*.webp` are team-owned K League photos (from the team Drive). Jjimjilbang card photo is `assets/photos/jjimjilbang/jjimjilbang-kiln-room.webp` — Wikimedia Commons **CC0** (public domain, no attribution needed, no people): source File:영신불가마사우나24시(강원 원주시 시청로 96)3.jpg. Per 유현님 (2026-07-28): jjimjilbang photos must not show other people's faces. `jjimjilbang-bulgama.webp` (also CC0) stays as an unused fallback; replace with the team's own photos once the event runs.
