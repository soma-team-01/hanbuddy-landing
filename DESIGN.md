...
- **Hero overlap layers** (mobile only): two separate absolutely positioned layers, both bleeding off the left and right edges. `.hero-lead` holds the top pair — the Jul 26 KBO group snap (`hp1`) and the K League crew selfie (`hp2`, swapped in for the first-baseball-night snap on 2026-08-22 so the newest run is visible on phones) — with the headline overlapping their lower half. `.hero-cta-band` holds the lower pair — the Banpo fountain snap (`hp3`) and the first-baseball-night snap (`hp4`, which took the slot of the Han River chimaek photo the same day) — in `.hero-polaroids` at CTA-button height, so the filled `Join a meetup` button stands between them. The band sits at button height rather than behind the body copy on purpose: thin `muted` body text cannot hold up over a photo, but a filled button and a bold link can, which is what lets the photos stay at 82% instead of being washed out. Captions are hidden in both layers because the frames are cropped by the viewport, so a handwritten caption would be cut mid-word.
...
- Two scheduling models exist in `event-slots.js` and the card must say which one it is. Fixed-slot activities (baseball, football) show date chips because tickets are bought for match days we do not control. Any-day activities showed `Any day you pick` instead (a specific date on those cards reads as a fixed schedule and hides the actual selling point); since 2026-09-04 no any-day activity is published (Han River, Korean BBQ and chimaek were retired), so the line is not rendered, but the rule returns with the model. Never hand-write a date on any card — chips are derived from `assets/event-slots.js` (`highlightDates()`) at render time, so a passed day disappears on its own.
- Event cards (`#events`, on the `panel` band shared with `#reviews`; one grid since 2026-09-04, two tiers between 2026-08-22 and then). **Sports nights** (`kbo-gocheok`, `kbo-jamsil`, `korea-football`; `kleague` until 2026-09-04, back when FC Seoul home games resume) are the push products: 1-col full-width on mobile / 3-col desktop, 2:1 photo on mobile (16/10 from `sm`), price badge on the image, emoji + title, no tagline at any width (the card stands on photo, title, and chips; the copy stays in CONTENT_MAP unrendered), then a date-chip row in chronological order starting at the `featured` day (else the nearest) — earlier open days are hidden on purpose so the chips never read backwards in time; the first chip wears `primary-soft`/`primary-strong`, the rest neutral with a `line-soft` ring, third chip desktop-only. No "+ more" tail: the full list belongs to the apply calendar. The **Food & chill** tier (compact 3-col at every width: square photo on mobile, smaller badge, small title, and the any-day line) was retired with the food activities on 2026-09-04; bring the tier back with the model if an any-day activity returns. The price appears once, on the badge only. Open events are whole-card links to `/events/*` detail pages; coming-soon events are buttons that fire a toast.
...
- Detail title block (2026-08-22 mobile-density redesign): eyebrow, `h1`, host line, then a `primary-soft` rating badge (`★ 4.8 (4 reviews)`, per-activity numbers, anchor to `#reviews`). Below the photo collage the content column opens with the hook `h2` and then a **key-facts table** (`<dl data-key-facts>`, one row per line at every width because these are the decision facts, `muted` label with an `aria-hidden` emoji, `ink` bold value): Venue (food nights have no fixed restaurant, so theirs reads `Seoul, exact spot sent by message after you apply`), Date (`Pick your date when you apply`), Meet, Fee (`₩65,000 per person` for baseball, `₩80,000` for the national team night; ₩60,000 until 2026-09-04), Included (ticket, food, listed separately from the fee because "₩60,000 · ticket & food" did not read; the venue-per-match national team page instead says the stadium is set by the match you pick and has no rating badge or review section yet), plus a Note row when an activity needs a caveat (pork). No Language row. Then **How the night/evening goes**: a numbered list of what actually happens (meet 1 hour before first pitch, walk in, grab food, first pitch, cheer and take photos), followed by one line saying we message the exact meeting spot after applying. No lead paragraph, no checkmark list, no "Apply → We confirm → Play ball" steps (they state the obvious), and no data-retention line (that notice lives in the form). 유현님 rules, 2026-08-22.
- Detail review section (`#reviews` on sports pages): "What guests say" heading with a `★ average · n reviews` line, then one flat `panel` card per rated review: an `aria-hidden` star row (`★`/`☆`, filled = survey score, `sr-only` "Rated n out of 5"), the verbatim quote, the anonymous attribution line. Reviews list every score, the 4 included, **newest run first** (`detailReviews()`), and every attribution line names the exact run date (`Jamsil, July 26, 2026`), never just the month. Gocheok borrows the Jamsil list under a visible `data-review-source-note`; a quote without a survey score never appears here. Content must mirror `assets/reviews-data.js` exactly (`tests/event-detail-reviews.test.js`).
...
- Maintainer-only guardrail: do not expose F001, 30,000, under 30,000, Less than 30,000, pre-acquaintance, local Korean interaction, proof of scale, learning signal, PMF caveats, payment sensitivity, or improvement criticism in public marketing copy. (The individual 4/5 survey score left this list on 2026-08-22: it is now deliberately public as a star row on the detail pages, sourced from `assets/reviews-data.js`.)
...
- The landing carousel keeps quote-only cards (the aggregate chip carries the stars here). Individual star rows moved to the event detail pages on 2026-08-22: with six scored reviews the earlier "never single out a lower score" worry flipped, and showing every score, the 4 included, reads as honesty rather than exposure.
...
- Final CTA (`#apply`) is the single large `ink` band over the photo backdrop: the `/apply/` form as the primary action, Instagram DM as the default guest inquiry channel, WhatsApp for international guests (added 2026-08-10), KakaoTalk chat as the local-buddy channel, and the one-line buddy-recruitment note.
