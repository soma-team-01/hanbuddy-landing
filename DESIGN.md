...
- Detail title block (2026-08-22 mobile-density redesign): eyebrow, `h1`, host line, then a `primary-soft` rating badge (`★ 4.8 (4 reviews)`, per-activity numbers, anchor to `#reviews`). The `data-key-facts` chip row (`panel` pills with an `aria-hidden` emoji each: place, meeting time, price + inclusions, `Pick your date when you apply`) sits directly **below the photo collage**, not in the title block: the photo is the hook and must stay in the first phone screen, the facts come right after it (유현님 rule, 2026-08-22). The chips replace the old place/time sentence; body copy is one hook `h2` plus a single paragraph, the checkmark list keeps only claims the chips do not already make, and "How joining works" is a compact numbered list, not three cards.
