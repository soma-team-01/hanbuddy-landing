...
- Detail title block (2026-08-22 mobile-density redesign): eyebrow, `h1`, host line, then a `primary-soft` rating badge (`★ 4.8 (4 reviews)`, per-activity numbers, anchor to `#reviews`) and a `data-key-facts` chip row (`panel` pills with an `aria-hidden` emoji each: place, meeting time, price + inclusions, `Pick your date when you apply`). The chips replace the old place/time sentence; body copy is one hook `h2` plus a single paragraph, the checkmark list keeps only claims the chips do not already make, and "How joining works" is a compact numbered list, not three cards.
- Detail review section (`#reviews` on sports pages): "What guests say" heading with a `★ average · n reviews` line, then one flat `panel` card per rated review: an `aria-hidden` star row (`★`/`☆`, filled = survey score, `sr-only` "Rated n out of 5"), the verbatim quote, the anonymous attribution line. Reviews list every score, the 4 included, in survey order. Gocheok borrows the Jamsil list under a visible `data-review-source-note`; a quote without a survey score never appears here. Content must mirror `assets/reviews-data.js` exactly (`tests/event-detail-reviews.test.js`).
