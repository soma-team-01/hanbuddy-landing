...
- Detail title block (2026-08-22 mobile-density redesign): eyebrow, `h1`, host line, then a `primary-soft` rating badge (`★ 4.8 (4 reviews)`, per-activity numbers, anchor to `#reviews`). Below the photo collage the content column opens with the hook `h2` and then a **key-facts table** (`<dl data-key-facts>`, one row per line at every width because these are the decision facts, `muted` label with an `aria-hidden` emoji, `ink` bold value): Venue (food nights have no fixed restaurant, so theirs reads `Seoul, exact spot sent by message after you apply`), Date (`Pick your date when you apply`), Meet, Fee (`₩60,000 per person`), Included (ticket, food, listed separately from the fee because "₩60,000 · ticket & food" did not read), plus a Note row when an activity needs a caveat (pork). No Language row. Then **How the night/evening goes**: a numbered list of what actually happens (meet 1 hour before first pitch, walk in, grab food, first pitch, cheer and take photos), followed by one line saying we message the exact meeting spot after applying. No lead paragraph, no checkmark list, no "Apply → We confirm → Play ball" steps (they state the obvious), and no data-retention line (that notice lives in the form). 유현님 rules, 2026-08-22.
