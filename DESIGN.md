...
- **Hero overlap layer** (`.hero-lead`, mobile only): the two KBO snaps (`hp1`, `hp2`) sit as an absolutely positioned layer at the top of the hero, bleeding off both edges, with the headline overlapping their lower half. The remaining two Han River snaps stay in `.hero-polaroids` below the copy. Captions are hidden here because the frames are cropped by the viewport, so a handwritten caption would be cut mid-word. Visual order is set with `order` on `.hero-copy` (title, eyebrow, actions, subtitle) while DOM order stays the reading order, so screen-reader and tab order are unchanged.
