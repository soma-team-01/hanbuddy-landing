...
⚠️ **이 시각은 집합 시간이지 경기 시작 시간이 아니다.** 규칙은 **평일 5:30, 주말 5:00 집합**이고 기존 회차가 전부 여기 맞는다. 구글폼은 이 값을 `Preferred time`이라고만 적어두어 신청자가 경기 시작 시각으로 오해할 여지가 있었으므로, 새 폼은 라벨과 슬롯 표기에 집합 시간임을 드러낸다(`Meet at 5:30 PM`). 요일에서 시각을 도출할 수 있지만 예외 회차가 생길 수 있으므로 슬롯마다 명시적으로 적는다.
