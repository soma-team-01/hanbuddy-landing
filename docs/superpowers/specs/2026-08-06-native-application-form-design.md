...
이메일을 보내지 않으므로 **신청자가 받는 확인은 이 화면이 전부다.** 구글폼의 "응답이 기록되었습니다" 한 줄을 확실히 넘어서야 한다. 담을 것은 셋이다.

1. **신청번호**(`HB-20260806-A3F9K2`). 문의할 때 댈 수 있는 번호가 있는 것과 없는 것은 체감이 다르다.
2. ⭐ **24시간 안에 연락해 자리를 확정한다는 약속.** 그 연락에서 참가 가능 여부, 결제, 집결 장소, 최종 일정을 정한다.
3. **인스타·카톡 바로가기.** 지금 문의할 곳.

**"신청 접수는 자리 확정이 아니다"라는 부정문은 쓰지 않는다**(유현님 결정, 2026-08-06). 방금 신청을 마친 사람에게 불안을 주고, 무슨 뜻인지 되묻게 만든다.

다만 **확정이 연락 이후라는 사실 자체는 남겨야 한다.** 8/1과 8/5를 인원 미달로 취소한 전례가 있어서, 자리를 확보했다고 믿은 사람이 다른 일정을 잡았다가 취소를 통보받으면 불만이 훨씬 커진다. 그래서 같은 사실을 24시간 약속에 실어 긍정문으로 쓴다.

```
We’ll be in touch within 24 hours to confirm your spot.
24시간 안에 연락드려 자리를 확정해 드립니다.
```

`confirm your spot` / `자리를 확정해 드립니다`가 "아직 확정 전"이라는 사실을 부정문 없이 전달한다.

**어느 채널로 연락하는지는 쓰지 않는다**(유현님 결정, 2026-08-06). 게스트는 WhatsApp·LINE·Instagram·WeChat 중에서 고르는데, 화면에 특정 앱 이름을 박으면 다른 걸 고른 사람에게는 틀린 문장이 된다. 방금 자기가 고른 수단이라 굳이 되풀이할 필요도 없다.

⚠️ **24시간은 팀이 지켜야 하는 약속이 된다.** 이 숫자를 쓰는 순간 디스코드 알림을 놓치면 곧바로 신뢰 손실로 이어진다. 알림 채널을 팀원이 실제로 보는 곳으로 두고, 응답 시간을 지키기 어려워지면 문구를 먼저 고친다. `AGENTS.md` Product facts에도 기록해 다른 채널과 어긋나지 않게 한다.
...
- 삭제·열람 요청 창구는 `zeroone.soma@gmail.com`.
- 기존 카피 중 사실이 아니게 되는 것을 전부 교체한다.
  - `events/*/index.html`: `This page never stores your application answers. You apply through the Google Form...`
...
**행사 종료 후 6개월**(유현님 결정, 2026-08-06). 동의는 **필수 하나**만 받고 마케팅용 선택 동의는 두지 않는다.

- 수집 목적이 그 회차 운영(참가 확인·결제·집결 안내·버디 배정)이라 목적 달성 시점이 행사일로 분명하다. 6개월은 거기에 환불·문의 대응 여유를 얹은 값이다.
- **그 이상 보관해도 실익이 없다.** 이메일을 받지 않기로 했으므로 남는 연락처는 메신저 ID뿐인데, 수명이 짧아 오래 지나면 대부분 닿지 않는다.
- MVP 오픈까지는 이 기간으로 덮인다. 8월 신청자 기준 2027년 2월까지이고 소마 본과정은 12월에 끝난다.

⚠️ **MVP 오픈 안내는 이 동의로 커버되지 않는다.** 회차 운영으로 받은 동의를 서비스 홍보에 쓰면 목적 외 이용이다. 그때 별도로 동의를 받거나, 안내 대상을 다시 모아야 한다. 선택 동의를 지금 넣지 않기로 한 만큼 이 제약은 감수한 것이다.

시트에는 `timestamp_kst`가 있으므로 파기 대상은 그 값으로 추린다. 자동 파기는 1차 범위 밖이고, 회차가 끝나고 6개월이 지난 행을 팀이 주기적으로 지운다.
...
| `AGENTS.md` | Product facts | 신청 후 **24시간 내 연락** 약속을 기록. 랜딩·구글폼·밋업이 같은 숫자를 써야 한다 |
...
3. ~~연락 응답 시간.~~ **해소됨(2026-08-06).** **24시간**으로 정했다. 완료 화면과 다른 채널 카피가 이 숫자를 함께 쓴다. 지키기 어려워지면 문구를 먼저 고친다.
4. ~~개인정보 보유기간.~~ **해소됨(2026-08-06).** 행사 종료 후 6개월, 필수 동의 하나(10.1). 파기는 수동이라 팀이 주기적으로 시트를 정리해야 한다.
